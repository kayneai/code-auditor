...
impl ToolExecutor {
    ...
    fn list_files(&self, args: &Value) -> ToolResult {
        ...
        let full_path = self.repo_root.join(dir);

        if !full_path.exists() {
            return ToolResult::error(format!("Directory not found: {}", dir));
        }

        if !full_path.is_dir() {
            return ToolResult::error(format!("Not a directory: {}", dir));
        }
        ...
        if let Ok(metadata) = std::fs::metadata(&full_path) {
            if metadata.len() > 100 * 1024 {
                return ToolResult::error(
                    "File too large (>100KB). Try reading a specific section.".to_string(),
                );
            }
        }
